...
class Bacteria:
    ...
    def bg_img(self):
        ...
        _, pimg = cv2.threshold(pimg, np.mean(pimg), 255, cv2.THRESH_BINARY)
        mask = (pimg/255).astype(np.uint8)
        return cv2.bitwise_and(self.part_img, self.part_img, mask=mask)
...
def generate_bacts(img, label, debug=False, debug_path = "for_debug.bmp", image_name = "current image", cover_corners=True, threshold = 2.0, max_diameter = 15):
    ...
    for bact in bacts:
        ...
        if (abs(bact.img_mean(bact.img) - bact.img_mean(bact.bg_img())) < threshold):
            continue
